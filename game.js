(() => {
  ...
  function fireBullet(fx, fy) {
    if (state.fireRounds <= 0) return;
    if (fx === undefined) fx = mouse.x;
    if (fy === undefined) fy = mouse.y;
    ...
    const bulletCount = 8 + ringLvl * 2;       // 8, 10, 12, 14
    const speed       = 600 + velLvl * 200;    // 600, 800, 1000
    const radius      = 4   + calLvl * 2;      // 4, 6, 8
    ...
    for (let i = 0; i < bulletCount; i++) {
      ...
      bullets.push({
        ...
        r: radius,
        life: 2,
        pierce, // hits remaining beyond the first kill
      });
    }
  ...
  canvas.addEventListener('touchstart', (e) => {
    ...
    for (const t of e.changedTouches) {
      ...
      if (touchState.moveId === null) {
        ...
        // Second-finger tap-fire at this finger's location
        if (state.running && !state.paused) fireBullet(p.x, p.y);
      }
    }
  }, { passive: false });
